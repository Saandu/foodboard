<template>
  ...
  <main v-else-if="store.isStructureLoaded" class="page"><section class="page-section"><section class="workspace-empty"><span>{{ $t('workspace_ready') }}</span><h1>{{ $t('create_first_menu_title') }}</h1><p>{{ $t('create_first_menu_text') }}</p><button type="button" class="btn btn-primary" :disabled="creating" @click="createFirstStructure">{{ $t(creating ? 'creating' : 'create_first_menu_button') }}</button></section></section></main>
</template>
